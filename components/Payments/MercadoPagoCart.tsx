...
const CheckoutMP = ({url, customerData, shippingData}: Props) => {
    ...
    return (
        <button onClick={handlePay} disabled={!mounted || loading} className={`bg-[#B86112] hover:bg-[#cb7818] font-semibold text-white px-4 py-2 rounded-lg transition-colors duration-300 ${mounted ? "cursor-pointer" : "cursor-no-drop"}`}>COMPRAR</button>
    );
}
